...
public class DrivingARouteAOverlay extends RouteAOverlay {
     ...
     */
    public DrivingARouteAOverlay(Context context, AMap amap, DrivePath path,
                                 LatLonPoint start, LatLonPoint end, List<LatLonPoint> throughPointList,int drable) {
    	super(context);
    	mContext = context;
        mAMap = amap;
        this.drivePath = path;
     ...
     * 添加驾车路线添加到地图上显示。
     */
	public void addToMap(int drable) {
		initPolylineOptions();
        try {
            if (mAMap == null) {
                return;
            ...
            for (DriveStep step : drivePaths) {
                ...
                tmcs.addAll(tmclist);
                addDrivingStationMarkers(step, convertToLatLng(latlonPoints.get(0)),drable);
...
//            addStartAndEndMarker();
     ...
     * @param latLng
     */
    private void addDrivingStationMarkers(DriveStep driveStep, LatLng latLng,int drable) {
        addStationMarker(new MarkerOptions()
                .position(latLng)
                .title("\u65B9\u5411:" + driveStep.getAction()
                        + "\n\u9053\u8DEF:" + driveStep.getRoad())
                .snippet(driveStep.getInstruction()).visible(nodeIconVisible)
                .anchor(0.5f, 0.5f).icon(BitmapDescriptorFactory.fromResource(drable)));
    }
    ...
    private BitmapDescriptor getThroughPointBitDes() {
    	return BitmapDescriptorFactory.fromResource(drable);
